			...
			foreach( $_mtphr_dnt_ticks as $i => $tick ) {
				...
				if( $text = wp_kses_post($tick['tick']) ) {
					...
					if( $link = esc_url($tick['link']) ) {
						...
						$contents = '<a href="'.$link.'" title="'. $text .'" target="'.$tick['target'].'"'.$nf.'>'.$text.'</a>';
					} else {
						$contents = $text;
					}
		...
		if( is_array($dnt_ticks) && $_mtphr_dnt_mode == 'rotate' ) {
			if( isset($_mtphr_dnt_rotate_directional_nav) ) {
				if( $_mtphr_dnt_rotate_directional_nav ) {
					...
					echo '<a class="mtphr-dnt-nav mtphr-dnt-nav-prev'.$hide.'" href="#" title="قبلی" rel="nofollow">'.apply_filters( 'mtphr_dnt_direction_nav_prev', '' ).'</a>';
					echo '<a class="mtphr-dnt-nav mtphr-dnt-nav-next'.$hide.'" href="#" title="بعدی" rel="nofollow">'.apply_filters( 'mtphr_dnt_direction_nav_next', '' ).'</a>';
				}
			}
		}
		echo '</div>';
		...
		if( is_array($dnt_ticks) && $_mtphr_dnt_mode == 'rotate' ) {
			if( isset($_mtphr_dnt_rotate_control_nav) ) {
				if( $_mtphr_dnt_rotate_control_nav ) {
					...
					foreach( $dnt_ticks as $i => $tick ) {
						echo '<a class="mtphr-dnt-control mtphr-dnt-control-'.$_mtphr_dnt_rotate_control_nav_type.'" href="'.$i.'" title="کنترل لینک ها" rel="nofollow">'.apply_filters( 'mtphr_dnt_control_nav', intval($i+1) ).'</a>';
					}
					echo '</div>';
				}
			}
