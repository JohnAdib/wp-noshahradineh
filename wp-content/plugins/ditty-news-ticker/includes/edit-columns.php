...
function mtphr_dnt_set_columns( $columns ){
	...
	foreach( $columns as $key => $value ) {
		if( $i == 2 ) {
			$new_columns['dnt_type'] = __( 'نوع', 'ditty-news-ticker' );
			$new_columns['dnt_mode'] = __( 'مدل', 'ditty-news-ticker' );
			$new_columns['dnt_shortcode'] = __( 'کد کوتاه', 'ditty-news-ticker' );
			$new_columns['dnt_function'] = __( 'تابع مستقیم', 'ditty-news-ticker' );
		}
